...
type HelperFunctionPlugin struct {
	newRuntime   func(context.Context) (wazero.Runtime, error)
	moduleConfig wazero.ModuleConfig
}

func NewHelperFunctionPlugin(ctx context.Context, opts ...wazeroConfigOption) (*HelperFunctionPlugin, error) {
	o := &WazeroConfig{
...
func (p *HelperFunctionPlugin) Load(ctx context.Context, pluginPath string) (helperFunction, error) {
	...
	if apiVersion == nil {
		return nil, errors.New("helper_function_api_version is not exported")
	}
	results, err := apiVersion.Call(ctx)
	if err != nil {
		return nil, err
	} else if len(results) != 1 {
		return nil, errors.New("invalid helper_function_api_version signature")
	}
	if results[0] != HelperFunctionPluginAPIVersion {
		return nil, fmt.Errorf("API version mismatch, host: %d, plugin: %d", HelperFunctionPluginAPIVersion, results[0])
	}
...
func (p *helperFunctionPlugin) Close(ctx context.Context) (err error) {
...
func (p *helperFunctionPlugin) Info(ctx context.Context, request *Empty) (*HelperFunctionInfo, error) {
	...
	response := new(HelperFunctionInfo)
...
func (p *helperFunctionPlugin) Execute(ctx context.Context, request *HelperFunctionRequest) (*HelperFunctionResponse, error) {
	data, err := request.MarshalVT()
	if err != nil {
		return nil, err
	}
	...
	response := new(HelperFunctionResponse)
	if err = response.UnmarshalVT(bytes); err != nil {
		return nil, err
	}
